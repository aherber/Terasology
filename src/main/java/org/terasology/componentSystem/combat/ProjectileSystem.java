...
public class ProjectileSystem implements EventHandlerSystem, UpdateSubscriberSystem {
    ...
    private void riccochette(Vector3f hitNormal,EntityRef projectile){
    	...
    	float margin = physicsSystem.getTrigggerForEntity(projectile).getCollisionShape().getMargin();
    	Vector3f reflect = new Vector3f();
    	hitNormal.normalize();
    	Vector3fUtil.reflect(projectileComponent.initialVelocity, hitNormal, reflect);
    	hitNormal.scale(margin);
    	reflect.add(hitNormal);
    	//check if translation/rotation is possible without another collision
    	LocationComponent location = projectile.getComponent(LocationComponent.class);
    	Vector3f position = new Vector3f();
    	float[] radius = new float[1];
    	physicsSystem.getTrigggerForEntity(projectile).getCollisionShape().getBoundingSphere(position, radius);
    	TriggerComponent trigger = projectile.getComponent(TriggerComponent.class);
    	HitResult hitResult = physicsSystem.getPhysics().rayTrace(location.getWorldPosition(), reflect, radius[0], physicsSystem.getPhysics().combineGroups(trigger.detectGroups));
    	if(hitResult.isHit()){
    		//TODO handle translation if collision will occur
    	}
...
//    		QuaternionUtil.quatRotate(rotation, direction, forward);
   ...
   */
    ...
    */
    ...
    /**
     * TODO this doesnt work until Particle effects have been fixed  
     */ 
    ...
    /**
     * Iterate through the collisions in the last frame and handlde the collisions
     */  
	...
	private void checkUpDown(CollideEvent event){
		
	}
	
	private void checkInfrontBehind(CollideEvent event){
		
	}
	
	
	//right = Vector3.Cross(direction, up);
	//mouseDir= mousePos-charPos Vector between hitPoint and characterPostion
}
